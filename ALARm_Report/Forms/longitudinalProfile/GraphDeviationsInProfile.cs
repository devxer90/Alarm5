...
{
    public class GraphDeviationsInProfile : Report
    {
        public override void Process(Int64 distanceId, ReportTemplate template, ReportPeriod period, MetroProgressBar progressBar)
        {
            ...
            using (XmlWriter writer = htReport.CreateWriter())
            {
                ...
                foreach (var process in mainProcesses.Where(o => o.Trip_id == 242).ToList())
                {
                    foreach (var track_id in admTracksId)
                    {
                        ...
                        if (RefPoints.Count() == 0 || RefPoints == null) continue;
                        ...
                        //rd_profile = rd_profile.Where(o => ((float)(float)filters[0].Value <= o.Km && o.Km <= (float)(float)filters[1].Value)).ToList();
                        //RefPoints = RefPoints.Where(o => ((float)(float)filters[0].Value <= o.Km && o.Km <= (float)(float)filters[1].Value)).ToList();
                        ...
                        for (int gi = 0; gi < new_original - 1; gi++)
                        {
                            ...
                            xePages.Add(
                                ...
                                new XAttribute("linearReper", gi < original["new_linear"].Count ? original["new_linear"][gi] : " ") // red

                            //new XAttribute("linearReper", gi < ReperLOlinear ? original["ReperLOlinear"][gi] : " ")  //red
                            );
                            ...
                            for (int i = 0; i < rp.Count; i++)
                            {
                                ...
                                try
                                {
                                    xePages.Add(new XElement("RefPoint",
                                        //Метка
                                        new XAttribute("xr", $"{(profileData.ReperX[rp[i].CoordAbs - 1]).ToString("0.####").Replace(',', '.')}"), //0.131 - это длина метки
                                        new XAttribute("yr", profileData.ReperY[rp[i].CoordAbs - 1]) //10 - это высота метки
                                        ));
                                }
                                catch (Exception e)
                                {
                                ...
                                xePages.Add(new XElement("straights_value",
                                    ...
                                    new XAttribute("y", $"{(profileData.ReperX[rp[i].CoordAbs - 1] + 0.075 < 0.15 ? 0.15 : profileData.ReperX[rp[i].CoordAbs - 1] + 0.075).ToString("0.####").Replace(',', '.')}"),
                                    new XAttribute("rotate", 270),
                                    new XAttribute("text", rp[i].Profile.ToString("0.##").Replace(',', '.'))));

                                //90 гр линия разделитель
                                xePages.Add(new XElement("straights90",
        ...
        public int GetIndexFromKmM(List<RdProfile> raw_rd_profile, int km, int m)
        {
